...
ActiveRecord::Schema.define(:version => 20120313202255) do
  ...
  create_view "view_tasks", "select `blocks`.`id` AS `id`,`blocks`.`type` AS `type`,`blocks`.`title` AS `title`,`blocks`.`comment` AS `comment`,`blocks`.`x` AS `x`,`blocks`.`y` AS `y`,`blocks`.`parent_id` AS `parent_id`,`blocks`.`game_id` AS `game_id`,`blocks`.`task_id` AS `task_id`,`blocks`.`created_at` AS `created_at`,`blocks`.`updated_at` AS `updated_at`,`tasks`.`name` AS `name` from (`blocks` join `tasks`) where (`blocks`.`id` = `tasks`.`id`)", :force => true do |v|
    v.column :id
    v.column :type
    v.column :title
    v.column :comment
